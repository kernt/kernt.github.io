...
When used together, Kubernetes, Vagrant, and Terraform can greatly simplify the process of deploying a Kubernetes cluster. Vagrant can be used to set up the necessary virtual machines for the cluster, while Terraform can provision the required infrastructure resources. Once the infrastructure is in place, Kubernetes can be deployed and managed using its own tools like _kubeadm_ or _kops_.
...
First clone the repository that provides the required configs[cloudspin](https://github.com/cloudspinx/virtualbox-kubernetes):

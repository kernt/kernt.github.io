...
2. Provides dynamic secret
3. Encryption as a service
4. Provide audit logs
5. Revocation is easy
...
- name: Creating vault user group  
  group:   
    name: "{{ vault_group }}"  
  become: true- name: Creating vault user  
  user:  
    name: "{{ vault_user }}"  
    group: "{{ vault_group }}"  
    system: yes  
...
- name: Install prerequisites  
  ...
  become: yes- name: Download binary  
  get_url:  
  ...
  register: vault_download- name: "Unzip vault archive"  
  unarchive:  
...
ConditionFileNotEmpty=/etc/vault.hcl[Service]  
User=vault  
Group=vault  
...
- name: Copy systemd init file  
  template:  
    ...
    owner: root  
    group: root  
  notify: systemd_reload- name: config file  
  template:  
    src: vault.hcl.j2  
    dest: "{{ vault_config_path }}"  
    owner: "{{ vault_user }}"  
    group: "{{ vault_group }}"- name: vault service  
  service:  
    name: vault  
    state: started  
    enabled: yes
...
- name: Create unseal directories  
  file:  
    ...
    state: directory  
  delegate_to: localhost- name: Create root key directories  
  file:  
    path: "{{ root_token_dir_output }}"  
    state: directory  
  delegate_to: localhost- name: Initialise Vault operator  
  shell: vault operator init -key-shares=5 -key-threshold=3 -format json  
  environment:  
    VAULT_ADDR: "[http://127.0.0.1:8200](http://127.0.0.1:8200)"  
  register: vault_init_results- name: Parse output of vault init  
  set_fact:  
    vault_init_parsed: "{{ vault_init_results.stdout | from_json }}"- name: Write unseal keys to files  
  copy:  
    dest: "{{ unseal_keys_dir_output }}/unseal_key_{{ item.0 }}"  
    content: "{{ item.1 }}"  
  with_indexed_items: "{{ vault_init_parsed.unseal_keys_hex }}"  
  delegate_to: localhost- name: Write root token to file  
  copy:  
...
- name: Reading unseal key contents  
  ...
  delegate_to: localhost  
  become: no- name: Unseal vault with unseal keys  
  shell: |  
...
`vault write secret/content value=secretDataSuccess! Data written to: secret/message`
...
```sh
vault read secret/contentKey                 Value---                 -----refresh_interval    768hvalue               secretData
```

...
Here the `collections.yml` content :
...
Here the `requirements.yml` content :

```yaml
---
roles: []

The converge playbook `molecule/default/molecule.yml` is redefined to include the WordPress role and the needed collection :
...
Just below you can find the content of the `molecule/common/Dockerfile.j2` template :
...
The apache user and group are grouped into `defaults/main.yml`to avoid repetition :
...
The `tasks/main.yml`file contains the configuration recipe of the WordPress role. [wp-cli](https://wp-cli.org/) to installs and configures WordPress when the container is started through an entrypoint file :
...
There is also a template file in `templates/php.ini.j2`to configure PHP settings :

`date.timezone = Europe/Paris`

The `files/entrypoint.sh`is a bash script passing variables from the docker command line to the container’s environment variables. The variables are then loaded to the WordPress server. This avoids storing sensitive data and having static configuration inside the Docker image :
...
We use TestInfra to ensure templates and files are well present with good permissions. It is located in `molecule/default/tests/test_default.py` :
